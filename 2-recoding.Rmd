...
eag_new_states = old_new_state_map_table %>% 
    ...
    dplyr::select(new_state) %>% dplyr::pull()
...
df$priority_state <- 1
df$priority_state[df$state %in% new_mappings[["Normal Focus"]]] <- 0
df$priority_state[df$state %in% new_mappings[["Other Focus_2"]]] <- 2
df$priority_state[df$state %in% new_mappings[["Other Focus_3"]]] <- 3
