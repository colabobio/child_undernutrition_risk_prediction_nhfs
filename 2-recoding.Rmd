...
```{r}
datafile <- here(data_folder, "merged.rda") 
load(datafile)
```
...
recoded_vars <- append(recoded_vars, "wgt2")
```
...
recoded_vars <- append(recoded_vars, "eligch")
```
...
recoded_vars <- append(recoded_vars, "defacto_child")
```
...
recoded_vars <- append(recoded_vars, "child_age")
```
...
recoded_vars <- append(recoded_vars, "age_group")
recoded_vars <- append(recoded_vars, "age_group_5y")
```
...
recoded_vars <- append(recoded_vars, "anthro_measured")
recoded_vars <- append(recoded_vars, "height_position")
```
...
recoded_vars <- append(recoded_vars, "haz") 
recoded_vars <- append(recoded_vars, "waz") 
recoded_vars <- append(recoded_vars, "whz")
recoded_vars <- append(recoded_vars, "bmiz")
recoded_vars <- append(recoded_vars, "stunted")
recoded_vars <- append(recoded_vars, "o_stunted")
```
...
recoded_vars <- append(recoded_vars, "underweight")
```
...
recoded_vars <- append(recoded_vars, "o_underweight")
```
...
recoded_vars <- append(recoded_vars, "wasted")
```
...
recoded_vars <- append(recoded_vars, "o_wasted")
```
...
recoded_vars <- append(recoded_vars, "ciaf")
```
...
recoded_vars <- append(recoded_vars, "stunt_waste")
```
...
recoded_vars <- append(recoded_vars, "elig")
```
...
recoded_vars <- append(recoded_vars, "wealth_quintile")
```
...
recoded_vars <- append(recoded_vars, "maternal_height_cm")
```
...
df <- df %>%
  mutate(maternal_height_4feet8inches_to_less_5feetinches = ifelse(df$maternal_height_cm > 142.24  & df$maternal_height_cm < 152.4, 1, 0) %>% as.factor(),
         ...
         maternal_height_5_2_feet_taller_inches = ifelse(df$maternal_height_cm > 152.4, 1, 0) %>% as.factor())

recoded_vars <- append(recoded_vars, "maternal_height_4feet8inches_to_less_5feetinches")
```
...
recoded_vars <- append(recoded_vars, "maternal_bmi")
```
...
recoded_vars <- append(recoded_vars, "paternal_height")
```
...
recoded_vars <- append(recoded_vars, "paternal_bmi")
```
...
recoded_vars <- append(recoded_vars, "hhadults")
recoded_vars <- append(recoded_vars, "hh_members")
```
...
recoded_vars <- append(recoded_vars, "m_age_marriage")
recoded_vars <- append(recoded_vars, "firstbirth_18y")
```
...
recoded_vars <- append(recoded_vars, "still_breastfed")
```
...
recoded_vars <- append(recoded_vars, "icfi_breastmilk_score")
```
...
recoded_vars <- append(recoded_vars, "icfi_bottlefeed_score")
```
...
df$icfi_starchy_staples_score <- NA
...
recoded_vars <- append(recoded_vars, "icfi_starchy_staples_score")
```
...
recoded_vars <- append(recoded_vars, "icfi_lentils_score")
```
...
recoded_vars <- append(recoded_vars, "icfi_milk_score")
```
...
recoded_vars <- append(recoded_vars, "icfi_dairy_score")
```
...
recoded_vars <- append(recoded_vars, "icfi_meat_score")
```
...
recoded_vars <- append(recoded_vars, "icfi_eggs_fish_score")
```
...
recoded_vars <- append(recoded_vars, "icfi_vitA_score")
```
...
recoded_vars <- append(recoded_vars, "icfi_other_fruits_score")
```
...
recoded_vars <- append(recoded_vars, "dietary_diversity_score")
```

xtile dd_quintile = dietary_diversity_score if child_age > = 6 & elig == 1, nq(4)

```{r}
# Assuming dietary_diversity_score, child_age, and elig are vectors in a data frame named 'df'

df <- df %>%
  mutate(dd_quintile = ifelse(child_age >= 6 & elig == 1,
                              {
                                # Calculate unique quantile breaks
                                quantiles <- unique(quantile(dietary_diversity_score[child_age >= 6 & elig == 1],
                                                             probs = seq(0, 1, by = 0.25), na.rm = TRUE))
                                
                                # Check if there are enough unique breaks to create the bins
                                if (length(quantiles) > 1) {
                                  as.numeric(cut(dietary_diversity_score,
                                                 breaks = quantiles,
                                                 include.lowest = TRUE))
                                } else {
                                  NA
...
recoded_vars <- append(recoded_vars, "bf_start")
```
...
recoded_vars <- append(recoded_vars, "infectious_disease")
```
...
recoded_vars <- append(recoded_vars, "water_source")
```
...
recoded_vars <- append(recoded_vars, "water_treatment")
```
...
recoded_vars <- append(recoded_vars, "stool_disposal")
```
...
recoded_vars <- append(recoded_vars, "toilet_type")
recoded_vars <- append(recoded_vars, "toliet_access_no")
```
...
recoded_vars <- append(recoded_vars, "toiletshare")
```
...
recoded_vars <- append(recoded_vars, "fuel_type")
```
...
recoded_vars <- append(recoded_vars, "cooking_fuel_solid")

levels(df$cooking_fuel) <- c("0. solid fuel", "1. non-solid fuel")
recoded_vars <- append(recoded_vars, "cooking_fuel")
```
...
recoded_vars <- append(recoded_vars, "kitchen")

df$separate_kitchen_no <- ifelse(df$kitchen == 0, 1,0)  %>% as.factor()
recoded_vars <- append(recoded_vars, "separate_kitchen_no")
```
...
recoded_vars <- append(recoded_vars, "air_quality")
```
...
recoded_vars <- append(recoded_vars, "vitamin_a")
```
...
recoded_vars <- append(recoded_vars, "child_anemia")
```
...
recoded_vars <- append(recoded_vars, "full_vaccination")
```
...
recoded_vars <- append(recoded_vars, "female")
```
...
recoded_vars <- append(recoded_vars, "sex_male")
```
...
recoded_vars <- append(recoded_vars, "total_children_born")
```
...
recoded_vars <- append(recoded_vars, "total_living_children")
recoded_vars <- append(recoded_vars, "siblings")
recoded_vars <- append(recoded_vars, "siblings_2")
recoded_vars <- append(recoded_vars, "siblings_bin")
recoded_vars <- append(recoded_vars, "birth_order")
```

```{r}
# Remove 'num_siblings_two_or_more' variable if it exists
...
recoded_vars <- append(recoded_vars, "num_siblings_two_or_more")
```
...
recoded_vars <- append(recoded_vars, "child_alive")
```
...
recoded_vars <- append(recoded_vars, "last_child_id")
```
...
recoded_vars <- append(recoded_vars, "sibling_under5")
```
...
recoded_vars <- append(recoded_vars, "pre_birth_interval")
```
...
recoded_vars <- append(recoded_vars, "pre_birth_interval_less_than_equal_to_24_months")
```
...
recoded_vars <- append(recoded_vars, "brothers_count")
recoded_vars <- append(recoded_vars, "brothers_home")
```
...
recoded_vars <- append(recoded_vars, "sisters_count")
recoded_vars <- append(recoded_vars, "sisters_home")
```
...
recoded_vars <- append(recoded_vars, "preg_deworm")
recoded_vars <- append(recoded_vars, "preg_ironyn")
recoded_vars <- append(recoded_vars, "preg_iron")
recoded_vars <- append(recoded_vars, "delivery_place")
recoded_vars <- append(recoded_vars, "child_deworm")
recoded_vars <- append(recoded_vars, "child_cough")
```
...
recoded_vars <- append(recoded_vars, "child_fever")
```
...
recoded_vars <- append(recoded_vars, "child_diarrhea")
```
...
recoded_vars <- append(recoded_vars, "birthweight")
```
...
df <- df %>%
  mutate(birth_weight_less_than_1800_g = ifelse(df$birthweight == "1. 1800g or less", 1, 0)  %>% as.factor(),
         ...
         birth_weight_more_than_2500_g = ifelse(df$birthweight == "3. more than 2500g", 1, 0) %>% as.factor())

recoded_vars <- append(recoded_vars, "birth_weight_less_than_1800_g")
recoded_vars <- append(recoded_vars, "birth_weight_1800_g_2500_g")
recoded_vars <- append(recoded_vars, "birth_weight_more_than_2500_g")
```
...
recoded_vars <- append(recoded_vars, "multiplet")
```
...
recoded_vars <- append(recoded_vars, "caste")
```
...
recoded_vars <- append(recoded_vars, "religion")
```
...
recoded_vars <- append(recoded_vars, "rural")
```
...
recoded_vars <- append(recoded_vars, "mother_education")
```
...
df <- df %>% 
  mutate(maternal_edu_no = ifelse(df$v133 == 0, 1, 0) %>% as.factor(),
         ...
         maternal_edu_greater_7_10 = ifelse(df$v133 >= 7 & df$v133 <= 10,
                              1, 0) %>% as.factor(),
         maternal_edu_greater_10 = ifelse(df$v133 > 10,
                              1, 0) %>% as.factor())

recoded_vars <- append(recoded_vars, "maternal_edu_no")
```
...
recoded_vars <- append(recoded_vars, "season")
```
...
recoded_vars <- append(recoded_vars, "anganwadi_support")
```
...
recoded_vars <- append(recoded_vars, "anc_visits")
```
...
recoded_vars <- append(recoded_vars, "covariate_nonmissing")
```
...
recoded_vars <- append(recoded_vars, "fem_aut_hc")
recoded_vars <- append(recoded_vars, "fem_aut_large")
recoded_vars <- append(recoded_vars, "fem_aut_daily")
recoded_vars <- append(recoded_vars, "fem_aut_visits")
```
...
##New code for the state ciaf rank based on the internal haven labelled data of state mapping
df <- df %>%
  mutate(state_ciaf_rank = case_when(
    state == 20 ~ "1. Jharkhand",
    state == 25 ~ "2. Dadra and Nagar Haveli and Daman and Diu",
    state == 10 ~ "3. Bihar",
    state == 23 ~ "4. Madhya Pradesh",
    state == 9  ~ "5. Uttar Pradesh",
    state == 24 ~ "6. Gujarat",
    state == 29 ~ "7. Karnataka",
    state == 22 ~ "8. Chhattisgarh",
    state == 8  ~ "9. Rajasthan",
    state == 27 ~ "10. Maharashtra",
    state == 17 ~ "11. Meghalaya",
    state == 6  ~ "12. Haryana",
    state == 21 ~ "13. Odisha",
    state == 5  ~ "14. Uttarakhand",
    state == 18 ~ "15. Assam",
    state == 19 ~ "16. West Bengal",
    state == 28 ~ "17. Andhra Pradesh",
    state == 34 ~ "18. Puducherry",
    state == 7  ~ "19. Delhi",
    state == 33 ~ "20. Tamil Nadu",
    state == 12 ~ "21. Arunachal Pradesh",
    state == 11 ~ "22. Sikkim",
    state == 36 ~ "23. Telangana",
    state == 35 ~ "24. Andaman and Nicobar Islands",
    state == 30 ~ "25. Goa",
    state == 3  ~ "26. Punjab",
    state == 2  ~ "27. Himachal Pradesh",
    state == 16 ~ "28. Tripura",
    state == 31 ~ "29. Lakshadweep",
    state == 1  ~ "30. Jammu and Kashmir",
    state == 4  ~ "31. Chandigarh",
    state == 32 ~ "32. Kerala",
    state == 13 ~ "33. Nagaland",
    state == 14 ~ "34. Manipur",
    state == 15 ~ "35. Mizoram",
    state == 37 ~ "36. Ladakh",   # <- added here
    TRUE ~ NA_character_
  ))
...
recoded_vars <- append(recoded_vars, "state")
recoded_vars <- append(recoded_vars, "state_ciaf_rank")
```


Old & New Mappings 
```{r}
old_state_mappings <- df %>%
  mutate(
    old_state = hv024 %>% as.numeric(),
    state_ciaf_rank = case_when(
      state == 15 ~ "1. Jharkhand",
      state == 8  ~ "2. Dadra and Nagar Haveli and Daman and Diu",
      state == 5  ~ "3. Bihar",
      state == 19 ~ "4. Madhya Pradesh",
      state == 33 ~ "5. Uttar Pradesh",
      state == 11 ~ "6. Gujarat",
      state == 16 ~ "7. Karnataka",
      state == 7  ~ "8. Chhattisgarh",
      ...
      state == 9  ~ "21. Dadra and Nagar Haveli and Daman and Diu",  # merged
      state == 3  ~ "22. Arunachal Pradesh",
      state == 30 ~ "23. Sikkim",
      state == 36 ~ "24. Telangana",
      state == 1  ~ "25. Andaman and Nicobar Islands",
      state == 10 ~ "26. Goa",
      state == 28 ~ "27. Punjab",
      state == 13 ~ "28. Himachal Pradesh",
      state == 32 ~ "29. Tripura",
      state == 18 ~ "30. Lakshadweep",
  ...
  dplyr::select(old_state, state_ciaf_rank) %>%
  distinct(old_state, .keep_all = TRUE)

new_state_mappings <- df %>%
  mutate(new_state = hv024 %>% as.numeric(),
         state_ciaf_rank = case_when(
    state == 20 ~ "1. Jharkhand",
    state == 25 ~ "2. Dadra and Nagar Haveli and Daman and Diu",
    state == 10 ~ "3. Bihar",
    ...
    state == 37 ~ "36. Ladakh",   # <- added here
    TRUE ~ NA_character_
  ))  %>% dplyr::select(c("new_state", "state_ciaf_rank")) %>%
  distinct(new_state, .keep_all = TRUE)

clean_name <- function(x) {
  sub("^\\d+\\.\\s*", "", x)
}

old_state_mappings = old_state_mappings %>% 
  dplyr::mutate(state_ciaf_rank = state_ciaf_rank %>% 
                  clean_name)

new_state_mappings = new_state_mappings %>% 
  dplyr::mutate(state_ciaf_rank = state_ciaf_rank %>% 
                  clean_name) 



old_new_state_map_table = full_join(old_state_mappings, 
                                     new_state_mappings, 
                                     by = "state_ciaf_rank") %>%
  dplyr::filter(!is.na(state_ciaf_rank))
```
...
#Mapping the old to new state labels
old_mappings = vector(mode = "list")
new_mappings = vector(mode = "list")

old_mappings[["North"]] = c(14, 28, 13, 6, 29, 25, 12)
old_mappings[["North-Eastern"]] = c(4, 3, 21, 22, 23, 24, 32, 30)
old_mappings[["Central"]] = c(7, 19, 34, 33)
old_mappings[["Eastern"]] = c(5, 15, 26, 35)
old_mappings[["Western"]] = c(8, 9, 10, 11, 20)
old_mappings[["Southern"]] = c(2, 16, 17, 18, 27, 31, 36, 1)

## Mapping to new mappings and making a mental check as well
for(loc in old_mappings %>% names()){
  
  new_mappings[[loc]] = old_new_state_map_table %>% 
    dplyr::filter(old_state %in% old_mappings[[loc]])
  
  print(paste0("------------", loc, "------------"))
  print(new_mappings[[loc]] %>% dplyr::select(state_ciaf_rank))
  
  new_mappings[[loc]] = new_mappings[[loc]] %>% 
...
df$state_zones <- case_when(
  df$v024 %in% new_mappings[["North"]] ~ 2, # North
  df$v024 %in% new_mappings[["North-Eastern"]] ~ 1, # North-Eastern
  df$v024 %in% new_mappings[["Central"]] ~ 3, # Central
  df$v024 %in% new_mappings[["Eastern"]] ~ 4, # Eastern
  df$v024 %in% new_mappings[["Western"]] ~ 5, # Western
  df$v024 %in% new_mappings[["Southern"]] ~ 6 # Southern
)
...
recoded_vars <- append(recoded_vars, "state_zones")
```
...
## Recoding the old state codes to the new ones
eag_old_state = c(4, 5, 6, 15, 19, 26, 29, 33, 34)
eag_new_states = old_new_state_map_table %>% 
    dplyr::filter(old_state %in% eag_old_state) %>% 
    dplyr::select(state_ciaf_rank) %>% dplyr::pull()

  

# Create a new "eag_state" column by recoding the "state" column
df$eag_state <- ifelse(
  df$state %in% eag_new_states,
  1,
  0
)

recoded_vars <- append(recoded_vars, "eag_state")
```
...
old_mappings[["Normal Focus"]] = c(5, 7, 13, 14, 15, 19, 26, 29, 33, 34)
old_mappings[["Other Focus_2"]] = c(3, 4, 21, 22, 23, 24, 30, 32)
old_mappings[["Other Focus_3"]] = c(1, 6, 8, 9, 18, 25, 27)
...
for(loc in old_mappings %>% names()){
  ...
  print(paste0("------------", loc, "------------"))
  old_new_state_map_table %>% 
          dplyr::filter(old_state %in% old_mappings[[loc]]) %>% print()
...
df$priority_state <- 1
df$priority_state[df$state %in% priority_old_mappings[["Normal Focus"]]] <- 0
df$priority_state[df$state %in% priority_old_mappings[["Other Focus_2"]]] <- 2
df$priority_state[df$state %in% priority_old_mappings[["Other Focus_3"]]] <- 3
...
recoded_vars <- append(recoded_vars, "priority_state")
recoded_vars <- append(recoded_vars, "priority_state_bin")
recoded_vars <- append(recoded_vars, "focus_states")
```
...
recoded_vars <- append(recoded_vars, "Northeast_focus")
recoded_vars <- append(recoded_vars, "Other_Focus")
```
...
recoded_vars <- append(recoded_vars, "water_home")
```
...
recoded_vars <- append(recoded_vars, "floor_finish")
```
...
recoded_vars <- append(recoded_vars, "wall_finish")
```
...
recoded_vars <- append(recoded_vars, "roof_finish")
```
...
recoded_vars <- append(recoded_vars, "house_finish")
```
...
recoded_vars <- append(recoded_vars, "use_soap")
```
...
recoded_vars <- append(recoded_vars, "christian")
```
...
recoded_vars <- append(recoded_vars, "low_caste")
```
...
recoded_vars <- append(recoded_vars, "toilet")
```
...
recoded_vars <- append(recoded_vars, "m_height")
```
...
recoded_vars <- append(recoded_vars, "iron_3mo")
```
...
recoded_vars <- append(recoded_vars, "maternal_education")
```
...
recoded_vars <- append(recoded_vars, "anc_4visits")
```
...
recoded_vars <- append(recoded_vars, "m_ht")
```
...
recoded_vars <- append(recoded_vars, "sibs")
```
...
recoded_vars <- append(recoded_vars, "house_partially_finished_no")
```
...
recoded_vars <- append(recoded_vars, "mother_agecat")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_house")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_mheight")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_medu")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_toilet")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_zone")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_fuel")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_female")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_caste")
```
...
recoded_vars <- append(recoded_vars, "scale_ciaf1_priority")
```
...
recoded_vars <- append(recoded_vars, "score_ciaf1")
```
...
```{r}
# shdistri was renamed to shdist in the latest DHS
...
recoded_vars <- append(recoded_vars, "dist_ciaf_mean")
recoded_vars <- append(recoded_vars, "mean_ciaf_dist")
recoded_vars <- append(recoded_vars, "dist_ciaf_decile")
recoded_vars <- append(recoded_vars, "ciaf_decile")
```
...
recoded_vars <- append(recoded_vars, "mean_district_CIAF_40_49.9")
recoded_vars <- append(recoded_vars, "mean_district_CIAF_50_59.9")
recoded_vars <- append(recoded_vars, "mean_district_CIAF_60_69.9")
recoded_vars <- append(recoded_vars, "mean_district_CIAF_70_79.9")
recoded_vars <- append(recoded_vars, "mean_district_CIAF_80_89.9")
```

```{r}
# Convert CIAF to factor after using is as numeric before
...
recoded_vars <- append(recoded_vars, "mean_wasted_dist")
recoded_vars <- append(recoded_vars, "dist_wasted_decile")
recoded_vars <- append(recoded_vars, "wasted_decile")
```


capture drop priority_district 
capture drop dist_priority
recode shdist (300 301 305 313 317 318 204  205 206 209 210 211 212 214 236 238 401 402 406 416 417 351 352 353 356 358 359 362 366 367 368 369 424 425 426 427 428 429 431 441 446 453 454 460 461 462 463 464 465 389 390 391 394 396 397 398 399 106 107 114 115 116 121 123 124 125 130 149 150 151 152 153 154 155 174 176 177 180 181 182 183 186 200 201 202 59 61 68 543 544 551 552 245 247 249 252 255 256 258 90 92 468 469 472 484 485 487 489 491 75 77 80 87 89 23 25 27 34 3 5 6 16 17 18 556 557 559 560 561 565 579 580 588 592 593 272 273 274 280 293 295 296 299 281 286 287 288 634 35 39 44 47 54 242 605 606 614 623 628 631 291 329 330 332 333 343 261 262 267 269 270 271 532 538 511 523 499 498 497 508 514 515 512 279 625 184 = 9999) , gen(dist_priority)
replace dist_priority = 0 if dist_priority != 9999
replace dist_priority = 1 if dist_priority == 9999

```{r}
# Create or modify the 'dist_priority' variable
df <- df %>%
  mutate(
    dist_priority = if_else(
      shdist %in% c(
        300, 301, 305, 313, 317, 318, 204, 205, 206, 209, 210, 211, 212, 214, 236, 238,
        401, 402, 406, 416, 417, 351, 352, 353, 356, 358, 359, 362, 366, 367, 368, 369,
        424, 425, 426, 427, 428, 429, 431, 441, 446, 453, 454, 460, 461, 462, 463, 464,
        465, 389, 390, 391, 394, 396, 397, 398, 399, 106, 107, 114, 115, 116, 121, 123,
        124, 125, 130, 149, 150, 151, 152, 153, 154, 155, 174, 176, 177, 180, 181, 182,
        183, 186, 200, 201, 202, 59, 61, 68, 543, 544, 551, 552, 245, 247, 249, 252,
        255, 256, 258, 90, 92, 468, 469, 472, 484, 485, 487, 489, 491, 75, 77, 80, 87,
        89, 23, 25, 27, 34, 3, 5, 6, 16, 17, 18, 556, 557, 559, 560, 561, 565, 579, 580,
        588, 592, 593, 272, 273, 274, 280, 293, 295, 296, 299, 281, 286, 287, 288, 634,
        35, 39, 44, 47, 54, 242, 605, 606, 614, 623, 628, 631, 291, 329, 330, 332, 333,
        343, 261, 262, 267, 269, 270, 271, 532, 538, 511, 523, 499, 498, 497, 508, 514,
        515, 512, 279, 625, 184
      ),
      1,  # Assign 1 for matching values
      0   # Assign 0 for non-matching values
    )
  )
...
df <- df %>% dplyr::filter(child_age < 60)

print(paste0("Filter only children under the age of 5 (60 months): ", nrow(df)))

df <- df %>% dplyr::filter(defacto_child == 1)

print(paste0("Children that are part of the household: ", nrow(df)))

df <- df %>% dplyr::filter(child_alive == 1)

print(paste0("Children that are alive: ", nrow(df)))

df <- df %>% dplyr::filter(!is.na(ciaf))

print(paste0("Children with complete anthropometric information: ", nrow(df)))
...
```{r}
# Save recoded data as R object
dffile <- here(data_folder, "recoded.rda") 
save(df, file=dffile)
```

```{r}
# Save list of recoded variables
rvfile <- here(data_folder, "recoded-variables.txt")
write.table(recoded_vars, rvfile, row.names=FALSE, col.names=FALSE)
```